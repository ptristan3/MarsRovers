...
class Rover(Rover_Interface):
  ...
  def process_mission(self, plateau):
    ...
    for command in self.mission:
      ...
      logging.info("Inicial Position %d %d %s, Command %s , Final Position %d %d %s", self.position.x, self.position.y, self.position.cardinal, str(command), new_position.x, new_position.y, new_position.cardinal)
      ...
      if plateau.is_valid_position(self.id_number, new_position):
        ...
        logging.info("Mission of Rover Number %d has finished", self.id_number)